...
func scanDir(t *testing.T, root string, goDirs, tsDirs []string) []string {
	t.Helper()
	keys, err := Scan(root, goDirs, tsDirs)
	if err != nil {
		t.Fatal(err)
	}
	return keys
...
func TestGoScanDetectsCyrillicLiteral(t *testing.T) {
	...
	keys := scanDir(t, root, []string{"internal"}, nil)
	want := []string{
...
func TestGoScanIgnoresComments(t *testing.T) {
	...
	if keys := scanDir(t, root, []string{"pkg"}, nil); len(keys) != 0 {
...
func TestGoScanDetectsEscapedUnicode(t *testing.T) {
	root := t.TempDir()
	writeFile(t, root, "pkg/a.go", "package pkg\n\nvar ru = \"\\u041f\\u0440\\u0438\\u0432\\u0435\\u0442\"\n")
	keys := scanDir(t, root, []string{"pkg"}, nil)
	if len(keys) != 1 {
...
func TestTSScanDetectsCyrillicString(t *testing.T) {
	...
	keys := scanDir(t, root, nil, []string{"src"})
...
func TestTSScanIgnoresLineComment(t *testing.T) {
	...
	if keys := scanDir(t, root, nil, []string{"src"}); len(keys) != 0 {
...
func TestTSScanDetectsAfterBlockCommentOnSameLine(t *testing.T) {
	...
	if keys := scanDir(t, root, nil, []string{"src"}); len(keys) != 1 {
...
func TestScanEmptyDirLists(t *testing.T) {
	...
	if keys := scanDir(t, root, nil, nil); len(keys) != 0 {
